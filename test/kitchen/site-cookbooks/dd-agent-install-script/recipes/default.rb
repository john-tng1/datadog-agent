...
kitchen_environment_variables = {
  ...
  'APT_URL' => node['dd-agent-install-script']['repo_domain_apt'],
  'YUM_URL' => node['dd-agent-install-script']['repo_domain_yum'],
  'APT_REPO_VERSION' => "#{node['dd-agent-install-script']['repo_branch_apt']} #{node['dd-agent-install-script']['repo_component_apt']}",
  'YUM_VERSION_PATH' => node['dd-agent-install-script']['repo_branch_yum'],
}.compact
