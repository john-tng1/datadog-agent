...
func newFixture(t *testing.T) *fixture {
	...
	f.store = NewDatadogMetricsInternalStore()
	return f
}
...
func newDatadogMetric(ns, name, query string, status datadoghq.DatadogMetricStatus) *datadoghq.DatadogMetric {
...
// We check that a leader controller only stores it locally to be processed.
func TestLeaderHandlingNewMetric(t *testing.T) {
	f := newFixture(t)
	metric := newDatadogMetric("default", "dd-metric-0", "metric query0", datadoghq.DatadogMetricStatus{})
...
func TestLeaderUpdateFromStoreInitialUpdate(t *testing.T) {
	...
	outputMetric := newDatadogMetric("", "dd-metric-0", "", datadoghq.DatadogMetricStatus{
		Value: 10.0,
